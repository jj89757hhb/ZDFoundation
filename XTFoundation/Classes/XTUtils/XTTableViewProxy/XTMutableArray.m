...
@interface XTArrayKeyedObjectSet : NSObject
...
- (NSUInteger)count;
@end
@implementation XTArrayKeyedObjectSet
- (instancetype)init {
    if ((self = [super init])) {
...
- (nullable NSArray *)objectsForID:(nonnull id)identity {
    XTArrayKeyedObjectSet *set = [self.mDictionary objectForKey:identity];
...
- (void)_setObjectToDictionaryIfNeeded:(id)object {
    if ([object conformsToProtocol:@protocol(XTModelID)]) {
        ...
        XTArrayKeyedObjectSet *set = [self.mDictionary objectForKey:identity];
        if (!set) {
            set = [[XTArrayKeyedObjectSet alloc] init];
...
- (id)mutableCopyWithZone:(NSZone *)zone {
    XTMutableArray *otherArray = [[[self class] allocWithZone:zone] init];
...
@implementation XTMutableArray (ObjectSubscripting)
- (id)objectForKeyedSubscript:(id)key {
    return [self firstObjectForID:key];
}
@end

@implementation NSArray (XTMutableArray)
- (XTMutableArray *)XTMutableArray {
    return [[XTMutableArray alloc] initWithArray:self];
}
@end
