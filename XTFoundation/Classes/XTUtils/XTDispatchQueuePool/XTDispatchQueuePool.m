...
static XTDispatchContext *XTDispatchContextCreate(const char *name,
                                                  uint32_t queueCount,
                                                  NSQualityOfService qos) {
    XTDispatchContext *context = calloc(1, sizeof(XTDispatchContext));
...
static void XTDispatchContextRelease(XTDispatchContext *context) {
    if (!context) return;
    if (context->queues) {
...
static dispatch_queue_t XTDispatchContextGetQueue(XTDispatchContext *context) {
...
static XTDispatchContext *XTDispatchContextGetForQOS(NSQualityOfService qos) {
    static XTDispatchContext *context[5] = {0};
    switch (qos) {
        case NSQualityOfServiceUserInteractive: {
            ...
            dispatch_once(&onceToken, ^{
                ...
                context[0] = XTDispatchContextCreate("com.xtwx.user-interactive", count, qos);
            });
            return context[0];
        } break;
        case NSQualityOfServiceUserInitiated: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[1] = XTDispatchContextCreate("com.xtwx.user-initiated", count, qos);
            });
            return context[1];
        } break;
        case NSQualityOfServiceUtility: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[2] = XTDispatchContextCreate("com.xtwx.utility", count, qos);
            });
            return context[2];
        } break;
        case NSQualityOfServiceBackground: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[3] = XTDispatchContextCreate("com.xtwx.background", count, qos);
            });
            return context[3];
        } break;
        ...
        default: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[4] = XTDispatchContextCreate("com.xtwx.default", count, qos);
            });
            return context[4];
        } break;
    }
}
@implementation XTDispatchQueuePool{
@public
    XTDispatchContext *_context;
}

- (void)dealloc {
    if (_context) {
        XTDispatchContextRelease(_context);
        _context = NULL;
    }
}

- (instancetype)initWithContext:(XTDispatchContext *)context {
    self = [super init];
...
- (instancetype)initWithName:(NSString *)name queueCount:(NSUInteger)queueCount qos:(NSQualityOfService)qos {
    ...
    self = [super init];
    _context = XTDispatchContextCreate(name.UTF8String, (uint32_t)queueCount, qos);
...
- (dispatch_queue_t)queue {
    return XTDispatchContextGetQueue(_context);
}

+ (instancetype)defaultPoolForQOS:(NSQualityOfService)qos {
    switch (qos) {
        case NSQualityOfServiceUserInteractive: {
            static XTDispatchQueuePool *pool;
            static dispatch_once_t onceToken;
            dispatch_once(&onceToken, ^{
                pool = [[XTDispatchQueuePool alloc] initWithContext:XTDispatchContextGetForQOS(qos)];
            });
            return pool;
        } break;
        case NSQualityOfServiceUserInitiated: {
        ...
        default: {
            ...
            static dispatch_once_t onceToken;
            dispatch_once(&onceToken, ^{
                pool = [[XTDispatchQueuePool alloc] initWithContext:XTDispatchContextGetForQOS(NSQualityOfServiceDefault)];
            });
            return pool;
        } break;
    }
...
dispatch_queue_t XTDispatchQueueGetForQOS(NSQualityOfService qos) {
    return XTDispatchContextGetQueue(XTDispatchContextGetForQOS(qos));
}
