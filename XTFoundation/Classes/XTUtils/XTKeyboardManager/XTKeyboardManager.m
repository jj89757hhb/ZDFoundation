...
static int _XTKeyboardViewFrameObserverKey;

/// Observer for view's frame/bounds/center/transform
@interface _XTKeyboardViewFrameObserver : NSObject
...
@implementation _XTKeyboardViewFrameObserver {
...
- (void)addToKeyboardView:(UIView *)keyboardView {
    ...
    if (_keyboardView) {
        ...
        objc_setAssociatedObject(_keyboardView, &_XTKeyboardViewFrameObserverKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _keyboardView = keyboardView;
    if (keyboardView) {
        [self addFrameObserver];
    }
    objc_setAssociatedObject(keyboardView, &_XTKeyboardViewFrameObserverKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
+ (instancetype)observerForView:(UIView *)keyboardView {
    ...
    return objc_getAssociatedObject(keyboardView, &_XTKeyboardViewFrameObserverKey);
}
...
@implementation XTKeyboardManager {
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"XTKeyboardManager init error" reason:@"Use 'defaultManager' to get instance." userInfo:nil];
    return [super init];
}
...
- (void)_initFrameObserver {
    ...
    _XTKeyboardViewFrameObserver *observer = [_XTKeyboardViewFrameObserver observerForView:keyboardView];
    if (!observer) {
        observer = [_XTKeyboardViewFrameObserver new];
...
+ (instancetype)defaultManager {
    static XTKeyboardManager *mgr = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (![UIApplication xt_isAppExtension]) {
...
- (UIWindow *)keyboardWindow {
    UIApplication *app = [UIApplication xt_sharedExtensionApplication];
    if (!app) return nil;
...
- (void)_notifyAllObservers {
    ...
    if (!CGRectEqualToRect(trans.toFrame, _fromFrame)) {
        for (id<XTKeyboardObserver> observer in _observers.copy) {
