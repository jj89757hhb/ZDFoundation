...
XTSYNTH_DUMMY_CLASS(UIImage_XTExtend)

@implementation UIImage (XTExtend)

//给定imageView尺寸，截取部分图像
- (UIImage *)xt_imageForSize:(CGSize)size {
    UIImage *image = [self xt_scaleToBigOfSize:CGSizeMake(size.width*2, size.height*2)];
    return image;
}
...
{
    return [self xt_resizedImageWithName:name left:0.5 top:0.5];
}
...
- (UIImage *)xt_imageForSize:(CGSize)size vertical:(XTImageVertical)vertical horizontal:(XTImageHorizontal)horizontal {
    ...
    if(imageRadio>sizeRadio) {//裁剪宽
        if(horizontal == XTImageHorizontalLeft){//左
            rect = CGRectMake(0, 0, height * sizeRadio, height);
        }else if (horizontal == XTImageHorizontalCenter){//中
        ...
        if(horizontal == XTImageVerticalTop){//左
            rect = CGRectMake(0, 0, width,width * size.height/size.width);
        }else if (horizontal == XTImageVerticalCenter){//中
...
+ (UIImage *)xt_imageByScalingToMaxSize:(UIImage *)sourceImage {
    ...
    return [self xt_imageByScalingAndCroppingForSourceImage:sourceImage targetSize:targetSize];
}

+ (UIImage *)xt_imageByScalingAndCroppingForSourceImage:(UIImage *)sourceImage targetSize:(CGSize)targetSize {
